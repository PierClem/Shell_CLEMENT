...
int pID,status,i; //pID est l'identifiant du processus tandis que status donne l'état du traitement du processus 
...
long elapsed=0; //Variable permettant d'obtenir le temps déroulé  
char* CHAINE[64];//Tableu de chaine de caractère 
...
int main()
{
	...
	{
		CHAINE[i] = strtok(buffer," "); //On sépare la chaine de caractère grâce a strtok
        while(CHAINE[i])
        CHAINE[++i] = strtok(NULL," ");//On incrémente la châine a séparer
        execvp(CHAINE[0],CHAINE);//On affiche les chaines séparées
//		execlp(buffer,CHAINE); //execlp permet comme précisé dans le cours d'exécuter une commande 
		exit(EXIT_SUCCESS);
	}
	clock_gettime(CLOCK_REALTIME,&End);
